# app.py (versão com a correção do aviso de deprecation)
...
async def processar_dataframe(df: pd.DataFrame, cep_column_name: str) -> pd.DataFrame: # (sem alterações)
    ...
    results = []
    for f in asyncio.as_completed(tasks): results.append(await f)
...
with st.sidebar:
    ...
    st.image("logo.png", use_container_width=True) # Trocamos 'use_column_width' por 'use_container_width'
    st.title("Capital Consig")
    st.info("Ferramenta oficial para validação e enriquecimento de CEPs em lote.")
...
uploaded_file = st.file_uploader(
    "Selecione o Arquivo de CEPs", type=["xlsx", "csv"], label_visibility="collapsed"
)
    ...
    try:
        ...
        if cep_col_name is None:
            ...
            if st.button(f"Processar {len(df)} Registros"):
                lottie_loading = load_lottieurl("https://assets5.lottiefiles.com/packages/lf20_stcrz6c5.json")
                with st.spinner("Executando consultas... As animações abaixo indicam o progresso."):
                    if lottie_loading: st_lottie(lottie_loading, speed=1, height=150, key="loading")
                ...
                nao_encontrados = (df_final['status_consulta'] == 'Não Encontrado').sum(); erros = total - sucessos - nao_encontrados
                col1, col2, col3 = st.columns(3)
                col1.metric("Total de Registros", total); col2.metric("Consultas com Sucesso", sucessos); col3.metric("Falhas de Consulta", erros)
                st.dataframe(df_final)
                excel_data = to_excel_bytes(df_final)
                st.download_button(
                    label="Baixar Planilha Processada (.xlsx)", data=excel_data,
