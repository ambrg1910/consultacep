# app.py (Versão 12.0 - O Portal Final: Estável, Polido e Pronto para Produção)
...
async def processar_dataframe_em_linhas(df: pd.DataFrame, cep_col_name: str, proposta_col_name: str) -> pd.DataFrame:
    ...
    # Inicia a sessão aqui, no escopo mais alto do processo assíncrono.
    async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT) as session:
        ...
        for index, row in df.iterrows():
            ...
            # Define a tarefa com todos os argumentos necessários. A 'session' é passada explicitamente.
            task = asyncio.ensure_future(fetch_and_format_lote(original_data, cep_p, session))
            tasks.append(task)
        
        placeholder = st.empty()
        for i, f in enumerate(asyncio.as_completed(tasks)):
            list_of_rows = await f
            all_new_rows.extend(list_of_rows)
    ...
    final_df.rename(columns={proposta_col_name: 'PROPOSTA', cep_col_name: 'CEP'}, inplace=True)
...
with tab_lote:
    ...
    st.info("Esta funcionalidade consulta 3 fontes de dados para cada CEP e retorna os resultados em múltiplas linhas.", icon="ℹ️")
