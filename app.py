# app.py (Versão 14.0 - A Versão Final de Produção: Arquitetura de Micro-Lotes)
...
BATCH_SIZE = 100          # Processaremos em lotes de 100 propostas por vez
PAUSE_BETWEEN_BATCHES = 5 # Pausa de 5 segundos entre cada lote
CONCURRENCY_PER_BATCH = 20 # Limite de requisições simultâneas dentro de um lote
REQUEST_TIMEOUT = 30       # Tempo de espera estendido para cada requisição
# ----------------------------------------------------
...
def display_api_status_dashboard():
    ...
    for col, (name, data) in zip(cols, statuses.items()):
        with col:
            ...
            st.metric(label=name, value=data['status'], delta=f"{data['latency']} ms" if data['latency'] >= 0 else "N/A", delta_color=delta_color, help="Tempo de Resposta. Quanto menor, melhor.")
...
async def processar_lote_industrial(df: pd.DataFrame, cep_col: str, prop_col: str) -> pd.DataFrame:
    ...
    async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT) as session:
        ...
        for batch_num, lote_df in enumerate(lista_de_lotes):
            ...
            for cep_results in results_do_lote:
                all_final_rows.extend(cep_results)
                
            total_propostas_processadas += len(lote_df)

            if batch_num + 1 < len(lista_de_lotes):
                for i in range(PAUSE_BETWEEN_BATCHES, 0, -1):
                    progress_bar.progress(total_propostas_processadas / len(df), text=f"Pausa estratégica de {i} segundos antes do próximo lote...")
...
with tab_individual:
    ...
    cep_input = col1.text_input("Digite o CEP:", max_chars=8, label_visibility="collapsed", placeholder="Digite o CEP (apenas números)")
    if col2.button("Consultar CEP"):
        if cep_input and len(cep_input) == 8 and cep_input.isdigit():
            resultados = consulta_cep_completa(cep_input); st.subheader("Resultados:")
            ...
            st.warning("Digite um CEP válido com 8 dígitos para consultar.")

with tab_lote:
    display_api_status_dashboard(); st.divider(); st.subheader("Processamento de Planilha em Lote")
    st.warning("Atenção: Esta funcionalidade é projetada para grandes volumes e pode levar vários minutos. Não feche esta aba durante o processamento.", icon="⏳")
    ...
    if uploaded_file:
        try:
            ...
            if not cep_col or not prop_col:
                ...
                st.success(f"Arquivo '{uploaded_file.name}' carregado. {len(df)} registros prontos para processar em {len(range(0, len(df), BATCH_SIZE))} lotes.")
                if st.button("Iniciar Processamento Industrial", use_container_width=True):
                    ...
                    st.download_button("Baixar Resultados Completos", to_excel_bytes(df_final), f"{uploaded_file.name.split('.')[0]}_RESULTADO_INDUSTRIAL.xlsx", use_container_width=True)
